-- Final security step: Restrict access to the legacy profiles table (Fixed)
...
-- Create a compatibility layer function that uses the new secure separated structure
...
DECLARE
    profile_data record;
BEGIN
    -- Use the new secure function and map it to the old format for backward compatibility
...
-- Add audit logging for modification attempts to the legacy profiles table
CREATE OR REPLACE FUNCTION public.log_legacy_profile_modification()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
AS $function$
BEGIN
    -- Log any direct modification to the legacy profiles table for security monitoring
    INSERT INTO public.audit_logs (
        ...
        jsonb_build_object(
            ...
            'warning', 'Direct modification to legacy profiles table detected',
            'recommendation', 'Use secure_update_separated_profile function instead',
            'timestamp', now()
        ),
        now()
    );
...
-- Add the audit trigger to track modifications (not SELECT operations)
DROP TRIGGER IF EXISTS log_legacy_profile_modification_trigger ON public.profiles;
CREATE TRIGGER log_legacy_profile_modification_trigger
    AFTER INSERT OR UPDATE OR DELETE ON public.profiles
    FOR EACH ROW
    EXECUTE FUNCTION public.log_legacy_profile_modification();