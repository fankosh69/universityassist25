...
-- Create simpler indexes for better performance
drop index if exists idx_cities_name_trgm;
drop index if exists idx_universities_name_trgm;
create index if not exists idx_cities_name_gin on public.cities using gin (name extensions.gin_trgm_ops);
create index if not exists idx_universities_name_gin on public.universities using gin (name extensions.gin_trgm_ops);