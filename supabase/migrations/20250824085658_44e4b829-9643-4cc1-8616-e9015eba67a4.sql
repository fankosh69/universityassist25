-- First, let's add some universities with proper city references
...
-- Now add programs referencing these universities
WITH university_refs AS (
  SELECT id, slug FROM universities WHERE slug IN ('humboldt-university-berlin', 'technical-university-munich', 'university-hamburg', 'goethe-university-frankfurt', 'university-stuttgart')
)
...
FROM (VALUES
  ...
  ('Biomedical Engineering', 'Bachelor of Engineering', 'bachelor', 'Engineering', 6, ARRAY['en', 'de'], 0, 180, true, 'biomedical-engineering-munich', 2.6, 'Interdisciplinary program combining engineering with medical applications', 'technical-university-munich'),
  ('International Management', 'Bachelor of Business Administration', 'bachelor', 'Business', 6, ARRAY['en'], 12000, 180, false, 'international-management-hamburg', 2.8, 'Global business management program with international focus', 'university-hamburg')
...
JOIN university_refs ON university_refs.slug = program_data.university_slug
ON CONFLICT (slug) DO NOTHING;