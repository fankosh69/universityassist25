-- Step 1: Create missing cities based on unique university cities
INSERT INTO cities (id, name, slug, country_code, created_at)
SELECT 
    gen_random_uuid() as id,
    u.city as name,
    slugify(u.city) as slug,
    'DE' as country_code,
    now() as created_at
FROM (
...
-- Step 3: Add coordinates to cities that don't have them (use first university's coordinates from that city)
UPDATE cities 
SET 
    lat = subq.avg_lat,
    lng = subq.avg_lng
