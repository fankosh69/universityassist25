...
BEGIN
  ...
  DO UPDATE SET
    ...
    curriculum = COALESCE(NULLIF(update_data->>'curriculum', ''), student_academics.curriculum),
    prev_major = COALESCE(NULLIF(update_data->>'prev_major', ''), student_academics.prev_major),
    target_level = COALESCE((update_data->>'target_level')::degree_level, student_academics.target_level),
    target_intake = COALESCE(NULLIF(update_data->>'target_intake', ''), student_academics.target_intake),
