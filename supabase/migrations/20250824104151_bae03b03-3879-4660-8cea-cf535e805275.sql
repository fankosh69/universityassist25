-- Fix Function Return Type Issue by Dropping and Recreating
-- This addresses the function signature conflict
...
ADD CONSTRAINT IF NOT EXISTS check_full_name_length CHECK (LENGTH(full_name) <= 100 OR full_name IS NULL);

ALTER TABLE public.profiles 
ADD CONSTRAINT IF NOT EXISTS check_current_institution_length CHECK (LENGTH(current_institution) <= 200 OR current_institution IS NULL);

-- 5. Create access validation function
...
-- 6. Update RLS policies to be ultra-restrictive
...
-- 7. Ensure restrictive table permissions
