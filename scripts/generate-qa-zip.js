...
async function generateQAZip() {
  try {
    console.log('🗜️  Generating QA packet ZIP...');
    
    const zip = new JSZip();
    
    // Add runbook.md
    const runbookPath = path.join(process.cwd(), 'testsprite', 'runbook.md');
    if (fs.existsSync(runbookPath)) {
      const runbookContent = fs.readFileSync(runbookPath, 'utf8');
      zip.file('runbook.md', runbookContent);
      console.log('✅ Added runbook.md');
    } else {
      console.warn('⚠️  runbook.md not found');
    }
...
`;
    ...
    const outputPath = path.join(process.cwd(), 'public', 'qa-packet.zip');
    fs.writeFileSync(outputPath, zipBuffer);
    
    console.log(`✅ QA packet ZIP generated: ${outputPath}`);
    console.log(`📦 Size: ${(zipBuffer.length / 1024).toFixed(2)} KB`);
    
    return outputPath;
  } catch (error) {
    console.error('❌ Failed to generate QA ZIP:', error);
    throw error;
  }
}
...
if (import.meta.url === `file://${process.argv[1]}`) {
  generateQAZip().catch(console.error);
}

export { generateQAZip };